...
int color_d(board_t board, int col){
    if(board[3][col]=='.'){
        return 0;
    }
    else if(board[2][col]=='.'){
...
{
    long i=0;
    i=(color_d(board,4)*1)+(color_d(board,3)*31)+(color_d(board,2)*31*31)+(color_d(board,1)*31*31*31)+(color_d(board,0)*31*31*31*31);
    return i;
}
...
void add_to_col(board_t board,int col,player_t player){
    for(int i=3;i>=0;i--){
        if(board[i][col]=='.'){
            board[i][col]=player;
            break;
...
void r_col(board_t board,int col){
    for(int i=0;i<4;i++){
        if(board[i][col]!='.'){
            board[i][col]='.';
            break;
...
move_t best_move(board_t board, player_t player)
{
    ...
    long o = ord(board);
    if (comp_moves[o]) {
        return decode_move(comp_moves[o]);
    }
    for(int col=0;col<5;col++){
        if(board[0][col]=='.'){
            add_to_col(board,col,player);
            if (has_won(board,player)){
                ...
                comp_moves[o] = encode_move(candidate = (move_t) {
                        .row = row,
                        .col = col,
                        .score = 1
                        });
    ...
    comp_moves[o] = encode_move(candidate);
    return candidate;
}
int main()
{
    ...
    while (1) {
        ...
        if (upd == 'R') {
            ...
            scanf("%d", &move);
            add_to_col(board,move,upd);
        } else{
            response = best_move(board, upd);
            printf("Computer move: %d\n",response.col);
            add_to_col(board,response.col,upd);
        }
        if (has_won(board, upd)) {
            print_board(board);
            if(upd=='R'){
                printf("You won!!\n");
            }
